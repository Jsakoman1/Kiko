...
- Prerequisites: KIKO-036 and KIKO-018.
...
<a id="kiko-041"></a>
### KIKO-041 — Start and resume one project thread

- Checkpoint kind: implementation
- Observable outcome: Adapter starts or resumes a thread bound to the selected workspace path.
- Why it matters: Repository-aware help must use the correct project without treating thread history as canonical state.
- Prerequisites: KIKO-040 and KIKO-018.
- Known concepts: Paths, IDs, state ownership, and validated messages.
- New concepts and syntax: Session-only identifiers and resume fallback.
- Learner task: Implement thread start/resume against fake lifecycle messages.
- Verification: `.venv/bin/python -m unittest tests.integration.test_app_server_thread -v`
- Expected behavior: Correct workspace is sent; valid session resumes; stale session creates a controlled new thread.
- Edge case: Thread IDs never enter learner competence or project progress data.
- Not included: Starting a turn or retaining raw conversation as project truth.
- Exit condition: Start, resume, stale-ID, and wrong-workspace fixtures pass.
...
- Prerequisites: KIKO-040 and KIKO-041.
...
- Prerequisites: KIKO-026, KIKO-027C, KIKO-035, and KIKO-042B.
...
- Prerequisites: KIKO-034A, KIKO-038B, and KIKO-042B.
...
- Prerequisites: KIKO-018B, KIKO-026, KIKO-035, and KIKO-043.
